...
- **SD card prep on macOS.** Do not use macOS `fdisk` to manually partition. The ROM silently rejects the result. Flash with `xz|dd` using the exact LP WIC image: `tisdk-default-image-am62xx-lp-evm-12.00.00.07.04.rootfs.wic.xz` — download from `https://dr-download.ti.com/software-development/software-development-kit-sdk/MD-PvdSyIiioq/12.00.00.07.04/tisdk-default-image-am62xx-lp-evm-12.00.00.07.04.rootfs.wic.xz`. balenaEtcher may fail on this file; use dd as primary. See `workspace/docs/BOOT_DAY_RUNBOOK.md §2`.
